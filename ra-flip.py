...
class s32int(int):
    ...
    def __new__(cls, value):
        ...
        if value >= (2 ** 31) or value < - (2 ** 31 - 1):
            ...
            orig_negative = 1 if value < 0 else 0

            negative = 1 if value & 2 ** 31 else 0
            if negative or orig_negative:
                value = - abs_value
            else:
                value = abs_value
...
class FlipObject:
    ...
    def graphicsItem(self):
        self.item = QtGui.QGraphicsRectItem(QtCore.QRectF(
            10 * self.x, 10 * self.y, 10, 10))
    ...
    def handle(self, ball):
        ...
        if mval:
            ...
            for i in range(0, 90):
                ...
                self.item.moveBy(9. / 90., 0)
                QtGui.qApp.processEvents()
            SlashWall(self.field, self.x, self.y, ":/bswall.svg")
            self.item.scene().removeItem(self.item)
...
class DownSluice(SVGObject):
    ...
    def handle(self, ball):
        ...
        if ball.sy > 0:
            ...
            if lmods:
                lval = eval('^'.join([str(mod.mvalue(ball.value))
                    for mod in lmods]))
            ...
            if rmods:
                rval = eval('^'.join([str(mod.mvalue(ball.value))
                    for mod in rmods]))
...
class UnicodeOut(TextObject):

    @Verbose(VERBOSE)
    def __init__(self, field, x, y):
        TextObject.__init__(self, field, x, y, 'U')
...
def autotest():
    ...
    debug("Modulo probado, %d tests fallaron, %d funcionaron debidamente" %
        (failed, passed))
    return failed
